 ...
 * @since JDK1.8
 */
public class FctpSubProblemPrimal extends AbstractFctpSubProblem {
    ...
    /**
     * Create a Instance FctpSubProblemPrimal.
     * 
     * @param fctpIns       问题实例
     * @param openVar 主问题中的 open 变量
     * @throws IloException
     */
    public FctpSubProblemPrimal(Fctp fctpIns, IloNumVar[] openVar) throws IloException {
        super(fctpIns);
        ...
        for (int j = 0; j < warehouseNum; j++) {
            for (int k = 0; k < customerNum; k++) {
                flows[j][k] = subSolver.numVar(0.0, Double.MAX_VALUE, "flow_" + j + "_" + k);
        ...
        /*
         * 容量约束: sum(flow[j][k], for k in customers) <= capacity[j] * open[j] Initially all zero, which
         * makes the subproblem infeasible
         */
        capacityCstr = new IloRange[warehouseNum];
        for (int j = 0; j < warehouseNum; j++) {
            capacityCstr[j] = subSolver.addLe(subSolver.sum(flows[j]), 0.0, "capacity_" + j);

            // 保存容量约束的右侧项，注意这里的右侧项存放的变量 open[j] * capacity[j]
            IloLinearNumExpr rhsExpr = subSolver.linearNumExpr();
            rhsExpr.addTerm(capacity[j], openVar[j]);
    ...
    @Override
    protected double getFlowBetween(int warehouseIndex, int customerIndex) throws UnknownObjectException, IloException {
